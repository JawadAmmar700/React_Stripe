...
import { CardNumberElement, CardExpiryElement, CardCvcElement, useStripe as Stripe, useElements as Elements } from '@stripe/react-stripe-js';
...
const checkoutForm = ({ username, email, pass }) => {
    ...
    Effect(() => {
        const getPermition = async () => {
            await axios({
                method: "POST",
                url: `http://localhost:5000/?price=${22 * 100}`
            }).then(res => {
    ...
    const Submit = async (e) => {
        ...
        await stripe.confirmCardPayment(paymentConf, {
            payment_method: {
        ...
        <>
            <form onSubmit={Submit} id="form" className="form">
                <div className="div">
                    ...
                    <CardNumberElement onChange={e => setNumError(e.error?.message)} className="CardNumber" />
                    <p>{numError}</p>
                </div>
                <div className="div">
                ...
                <div className="div">
                    ...
                    <CardExpiryElement onChange={e => setDateError(e.error?.message)} className="CardExp" />
                    <p>{dateError}</p>
                </div>
                <button type="submit" disabled={disable || loading}>
                    {loading ? <span>
                        <img id="img" src="https://image.flaticon.com/icons/png/128/39/39979.png" alt="" /> processing </span> : 'Pay 22$'}
